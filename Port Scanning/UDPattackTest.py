...
class PacketCreatorTest( unittest.TestCase ):
    ...
    def test_argsPacketsCreator_UDPattack( self ):
        puertos=[[80,25,137,1024,53],[161,123,111,500,69,28960,19,9987,5353,12203,2049,9915,63392,520]]
        ataque=argsPacketsCreator( '200.7.4.7', ['56.145.96.4'], [37599], puertos, 13, 25, 13, 9, False, 0, 1 )
        self.assertEqual( len( ataque ), 13, 'error en la cantidad de paquetes en el ataque: Script "PacketCreator", funcion "argsPacketsCreator" seccion "UDP attack"' )

        for i in range( len( ataque ) ):
            self.assertEqual( ataque[i][0], 37599, 'error en el puerto de la pregunta: Script "PacketCreator", funcion "argsPacketsCreator" seccion "UDP attack"' )
            self.assertTrue( ( ataque[i][1] in puertos[0] ) or ( ataque[i][1] in puertos[1] ), 'error en el puerto de la pregunta: Script "PacketCreator", funcion "argsPacketsCreator" seccion "UDP attack"' )
            self.assertEqual( ataque[i][3], '56.145.96.4', 'error en la direccion IP de origen en el paquete IP: Script "PacketCreator", funcion "argsPacketsCreator" seccion "UDP attack"' )
            self.assertEqual( ataque[i][4], '200.7.4.7', 'error en la direccion IP de destino en el paquete IP: Script "PacketCreator", funcion "argsPacketsCreator" seccion "UDP attack"' )
            self.assertTrue( ataque[i][5]<=25-0.13 and ataque[i][5]>=13, 'error en el tiempo del ataque: Script "PacketCreator", funcion "argsPacketsCreator" seccion "UDP attack"' )


    def test_argsPacketsCreator_UDPattack_icmp( self ):
        puertos = [[80,25,137,1024,53],[161,123,111,500,69,28960,19,9987,5353,12203,2049,9915,63392,520]]
        ips = ['39.199.1.200', '195.23.145.200']
        portsrc = [200,145]
        args = argsPacketsCreator( '200.7.4.7', ips, portsrc, puertos, 0, 120, 80, 0, 1, 3, 1 )
        ataque = []
        ...
        for i in range( len( ataque ) ):
            if args[i][2]:
                c += 1
                t.append( args[i][5] )
                self.assertEqual( len( ataque[i] ), 2, 'error en el largo del array pregunta-respuesta: Script "PacketCreator", funcion "argsPacketsCreator" seccion "UDP attack"' )
                if c>=4:
                    self.assertTrue( ( t[c-1]-t[c-3] )<=60 )
            else:
                self.assertEqual( len( ataque[i] ), 1, 'error en el largo del array pregunta-respuesta: Script "PacketCreator", funcion "argsPacketsCreator" seccion "UDP attack"' )



    def test_generadorParesPortScanningUDP( self ):
        puertos=[[80,25,137,1024,53],[161,123,111,500,69,28960,19,9987,5353,12203,2049,9915,63392,520]]
        args=argsPacketsCreator( '200.7.4.7', ['56.145.96.4'], [37599], puertos, 13, 25, 13, 9, False, 0, 1 )
        ataque = []
        for i in range( len( args ) ):
            ataque.append( generadorParesPortScanningUDP( args[i] ) )
    ...
    def test_UDP_DDoS_attack( self ):
        ...
        packAt=argsPacketsCreator( '200.7.4.7', ips, ports, [[],list( range( 25,80,2 ) )], 160, 162, 40, 5, False, 2, 1 )
    ...
    def test_UDP_Attack( self ):
        ...
        packAt=argsPacketsCreator( '200.7.4.7', ['56.145.96.4'], [37599], puertos, 13, 25, 300, 9, False, 2, 1 )
